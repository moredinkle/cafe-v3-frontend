<template>
  ...
  const inputClass = computed(() => {
    return props.error
      ...
      : "w-full border px-2 py-1 shadow-sm dark:bg-slate-800 rounded-md rounded-s-none border-s-0 border focus:ring-blue-600";
  });
  const iconClass = computed(() => {
    return props.error
      ...
      : "inline-flex items-center px-3 text-gray-900 bg-gray-200 border rounded-e-0 border-gray-300 border-e-0 rounded-s-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600 border focus:ring-blue-600";
  });
  </script>
